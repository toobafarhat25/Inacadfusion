...
const BrowseProjects = () => {
  ...
  const floatingNodes = [
    { label: 'Ansys', top: '12%', left: '8%', delay: 0 },
    { label: 'Aidoc', top: '48%', left: '4%', delay: 0.5 },
    { label: 'Canva', top: '75%', left: '16%', delay: 1 },
    { label: 'Google', top: '10%', right: '10%', delay: 0.3 },
    { label: 'GitHub', top: '42%', right: '5%', delay: 0.8 },
    { label: 'Python', top: '70%', right: '14%', delay: 1.2 }
  ]
  ...
  const loadProjects = async (isManual = false) => {
    ...
    try {
      if (isAiMode && (search || isManual)) {
        ...
        if (domainFilter === 'All' && !search) {
          const uniqueDoms = Array.from(new Set(res.data.data.map(p => p.domain))).filter(Boolean)
  ...
  // Filter local listings by publisher/sector
  const filteredProjects = projects.filter(project => {
    if (roleFilter === 'All') return true
    return project.uploadedBy?.role === roleFilter
  })
        ...
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 2 }}>
          ...
          <Box className="vx-hero-section">
            <Typography variant="h1" className="vx-hero-title">
              ...
              that you need for your Industry
            </Typography>
            <Typography className="vx-hero-desc">
              InAcadFusion is a search and recommendation platform that connects students and startups. Describe your tech stack or key interests below to find matching milestones.
            </Typography>
            ...
            <form onSubmit={handleSearchSubmit}>
              <Box className="vx-search-pill">
                ...
                <TextField
                  fullWidth
                  placeholder={isAiMode ? "Describe your skills & interests (e.g. 'I know React')...." : "Search project title or keywords..."}
                  value={search}
          ...
          <Box className="vx-filter-row">
            ...
            {/* Card 2: Filter by sector */}
            <Box className="vx-filter-card-dark">
              <Box>
                <Box className="vx-card-title-row">
                  <h3>Filter By Sector</h3>
                  {roleFilter !== 'All' && (
                    <Button className="vx-card-link-btn" onClick={() => setRoleFilter('All')}>See all</Button>
                  )}
                </Box>
                <Box className="vx-filter-buttons-grid">
                  <Button 
                    className={`vx-filter-btn-item ${roleFilter === 'All' ? 'active' : ''}`}
                  ...
                  <Button 
                    className={`vx-filter-btn-item ${roleFilter === 'startup' ? 'active' : ''}`}
                    onClick={() => setRoleFilter('startup')}
                    startIcon={<Business sx={{ fontSize: 16 }} />}
                  >
                    Startup Partner
                  </Button>
                  <Button 
                    className={`vx-filter-btn-item ${roleFilter === 'student' ? 'active' : ''}`}
            ...
            {loading ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <Box className="vx-project-card" sx={{ p: 6, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                  <EmptyState icon="projects" title="No projects found" message="Try relaxing your filters or modifying the search terms." />
                </Box>
              </motion.div>
            ) : (
              <Grid container spacing={4}>
                        ...
                        <div className="vx-project-card">
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              ...
                              {project.confidence && (
                                <div className="vx-card-badge">
                                  <AutoAwesome sx={{ fontSize: 13, mr: 0.5, verticalAlign: 'middle' }} /> {project.confidence}% Match
                                </div>
                              )}
                            </Box>
                          </div>
                          ...
                          <div className="vx-card-body">
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 'auto' }}>
                              <Domain sx={{ fontSize: 18, color: '#94A3B8' }} />
                          ...
                          <div className="vx-card-footer">
                            <Button 
                              ...
                              onClick={() => navigate(`/student/project/${createSlug(project.title, project.id || project._id)}`)}
                            >
                              View Opportunity
                            </Button>
                          </div>
